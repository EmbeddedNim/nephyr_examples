...
template SPI_OP_MODE_GET*(_operation_: untyped): untyped =
  ((_operation_) and SPI_OP_MODE_MASK)
...
template SPI_MODE_GET*(_mode_: untyped): untyped =
  ((_mode_) and SPI_MODE_MASK)
...
const
  SPI_WORD_SIZE_SHIFT* = (5)
  SPI_WORD_SIZE_MASK* = (0x3F shl SPI_WORD_SIZE_SHIFT)

template SPI_WORD_SIZE_GET*(_operation_: untyped): untyped =
  (((_operation_) and SPI_WORD_SIZE_MASK) shr SPI_WORD_SIZE_SHIFT)

template SPI_WORD_SET*(_word_size_: untyped): untyped =
  ((_word_size_) shl SPI_WORD_SIZE_SHIFT)
...
const
  SPI_LINES_SINGLE* = (0 shl 11)
  SPI_LINES_DUAL* = (1 shl 11)
  SPI_LINES_QUAD* = (2 shl 11)
  SPI_LINES_OCTAL* = (3 shl 11)
...
type
  spi_cs_control* {.importc: "spi_cs_control", header: "spi.h", bycopy.} = object
...
template SPI_CS_CONTROL_PTR_DT*(node_id, delay_: untyped): untyped =
  (addr(cast[spi_cs_control]((gpio_dev: DEVICE_DT_GET(
      DT_SPI_DEV_CS_GPIOS_CTLR(node_id)), delay: (delay_),
...
template SPI_CS_CONTROL_PTR_DT_INST*(inst, delay_: untyped): untyped =
...
type
  spi_config* {.importc: "spi_config", header: "spi.h", bycopy.} = object
...
template SPI_CONFIG_DT*(node_id, operation_, delay_: untyped): void =
  ## !!!Ignored construct:  . frequency = DT_PROP ( node_id , spi_max_frequency ) , . operation = ( operation_ ) , . slave = DT_REG_ADDR ( node_id ) , . cs = COND_CODE_1 ( DT_SPI_DEV_HAS_CS_GPIOS ( node_id ) , ( SPI_CS_CONTROL_PTR_DT ( node_id , delay_ ) ) , ( NULL ) ) , }
...
template SPI_CONFIG_DT_INST*(inst, operation_, delay_: untyped): untyped =
...
type
  spi_buf* {.importc: "spi_buf", header: "spi.h", bycopy.} = object
...
type
  spi_buf_set* {.importc: "spi_buf_set", header: "spi.h", bycopy.} = object
...
type
  spi_driver_api* {.importc: "spi_driver_api", header: "spi.h", bycopy.} = object
    transceive* {.importc: "transceive".}: spi_api_io
    when defined(CONFIG_SPI_ASYNC):
      var transceive_async* {.header: "spi.h".}: spi_api_io_async
...
proc spi_transceive*(dev: ptr device; config: ptr spi_config; tx_bufs: ptr spi_buf_set;
    ...
    importc: "spi_transceive", header: "spi.h".}
proc z_impl_spi_transceive*(dev: ptr device; config: ptr spi_config;
                           tx_bufs: ptr spi_buf_set; rx_bufs: ptr spi_buf_set): cint {.
    inline, importc: "z_impl_spi_transceive".} =
...
proc spi_release*(dev: ptr device; config: ptr spi_config): cint {.syscall,
    importc: "spi_release", header: "spi.h".}
proc z_impl_spi_release*(dev: ptr device; config: ptr spi_config): cint {.inline,
    importc: "z_impl_spi_release".} =
  let api: ptr spi_driver_api
