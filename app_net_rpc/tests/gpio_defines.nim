...
const
  GPIO_INPUT* = (1'u shl 8)

## * Enables pin as output, no change to the output state.

const
  GPIO_OUTPUT* = (1'u shl 9)
...
const
  GPIO_OUTPUT_INIT_LOW* = (1'u shl 10)

##  Initializes output to a high state.

const
  GPIO_OUTPUT_INIT_HIGH* = (1'u shl 11)

##  Initializes output based on logic level

const
  GPIO_OUTPUT_INIT_LOGICAL* = (1'u shl 12)

## * @endcond
## * Configures GPIO pin as output and initializes it to a low state.

const
  GPIO_OUTPUT_LOW* = (GPIO_OUTPUT or GPIO_OUTPUT_INIT_LOW)

## * Configures GPIO pin as output and initializes it to a high state.

const
  GPIO_OUTPUT_HIGH* = (GPIO_OUTPUT or GPIO_OUTPUT_INIT_HIGH)

## * Configures GPIO pin as output and initializes it to a logic 0.

const
  GPIO_OUTPUT_INACTIVE* = (
    GPIO_OUTPUT or GPIO_OUTPUT_INIT_LOW or GPIO_OUTPUT_INIT_LOGICAL)

## * Configures GPIO pin as output and initializes it to a logic 1.

const
  GPIO_OUTPUT_ACTIVE* = (
    GPIO_OUTPUT or GPIO_OUTPUT_INIT_HIGH or GPIO_OUTPUT_INIT_LOGICAL)
...
const
  GPIO_INT_DISABLE* = (1'u shl 13)

## * @cond INTERNAL_HIDDEN
##  Enables GPIO pin interrupt.

const
  GPIO_INT_ENABLE* = (1'u shl 14)
...
template GPIO_DT_SPEC_GET_BY_IDX_OR*(node_id, prop, idx, default_value: untyped): untyped =
...
template GPIO_DT_SPEC_GET*(node_id, prop: untyped): untyped =
...
template GPIO_DT_SPEC_GET_OR*(node_id, prop, default_value: untyped): untyped =
  GPIO_DT_SPEC_GET_BY_IDX_OR(node_id, prop, 0, default_value)
...
template GPIO_DT_SPEC_INST_GET_BY_IDX*(inst, prop, idx: untyped): untyped =
...
template GPIO_DT_SPEC_INST_GET_BY_IDX_OR*(inst, prop, idx, default_value: untyped): untyped =
...
template GPIO_DT_SPEC_INST_GET*(inst, prop: untyped): untyped =
...
template GPIO_DT_SPEC_INST_GET_OR*(inst, prop, default_value: untyped): untyped =
  GPIO_DT_SPEC_INST_GET_BY_IDX_OR(inst, prop, 0, default_value)
...
const
  GPIO_MAX_PINS_PER_PORT* = (sizeof((gpio_port_pins_t) * __CHAR_BIT__))
