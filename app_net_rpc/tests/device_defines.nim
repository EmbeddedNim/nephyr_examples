...
template DEVICE_NAME_GET*(name: untyped): untyped =
  _CONCAT(__device_, name)
...
template SYS_DEVICE_DEFINE*(drv_name, init_fn, pm_control_fn, level, prio: untyped): untyped =
  DEVICE_DEFINE(Z_SYS_NAME(init_fn), drv_name, init_fn, pm_control_fn, nil, nil,
                level, prio, nil)
...
template DEVICE_DEFINE*(dev_name, drv_name, init_fn, pm_control_fn, data_ptr, cfg_ptr,
                       level, prio, api_ptr: untyped): untyped =
  Z_DEVICE_DEFINE(DT_INVALID_NODE, dev_name, drv_name, init_fn, pm_control_fn,
...
template DEVICE_DT_NAME*(node_id: untyped): untyped =
...
template DEVICE_DT_DEFINE*(node_id, init_fn, pm_control_fn, data_ptr, cfg_ptr, level,
                          prio, api_ptr: untyped; xargs: varargs[untyped]): untyped =
  Z_DEVICE_DEFINE(node_id, Z_DEVICE_DT_DEV_NAME(node_id), DEVICE_DT_NAME(node_id),
                  init_fn, pm_control_fn, data_ptr, cfg_ptr, level, prio, api_ptr,
                  __VA_ARGS__)
...
template DEVICE_DT_INST_DEFINE*(inst: untyped; xargs: varargs[untyped]): untyped =
...
template DEVICE_DT_NAME_GET*(node_id: untyped): untyped =
  DEVICE_NAME_GET(Z_DEVICE_DT_DEV_NAME(node_id))
...
template DEVICE_DT_GET*(node_id: untyped): untyped =
  (addr(DEVICE_DT_NAME_GET(node_id)))
...
template DEVICE_DT_INST_GET*(inst: untyped): untyped =
...
template DEVICE_DT_GET_ANY*(compat: untyped): untyped =
  COND_CODE_1(DT_HAS_COMPAT_STATUS_OKAY(compat),
              (DEVICE_DT_GET(DT_COMPAT_GET_ANY_STATUS_OKAY(compat))), (nil))
...
template DEVICE_GET*(name: untyped): untyped =
  (addr(DEVICE_NAME_GET(name)))
...
template Z_DEVICE_DT_DEV_NAME*(node_id: untyped): untyped =
...
template Z_DEVICE_STATE_NAME*(dev_name: untyped): untyped =
  _CONCAT(__devstate_, dev_name)
...
template Z_DEVICE_HANDLE_NAME*(node_id, dev_name: untyped): untyped =
  _CONCAT(__devicehdl_,
          COND_CODE_1(DT_NODE_EXISTS(node_id), (node_id), (dev_name)))

template Z_DEVICE_EXTRA_HANDLES*(: untyped; xargs: varargs[untyped]): untyped =
...
template Z_DEVICE_DEFINE_PRE*(node_id, dev_name: untyped; xargs: varargs[untyped]): untyped =
